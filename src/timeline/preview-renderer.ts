...
export class PreviewRenderer {
  ...
  public async play(): Promise<void> {
    ...
    this._playbackStartTime = performance.now() - (this._timeline.currentTime * 1000);
    ...
    const animate = async (timestamp: number) => {
      ...
      const currentTime = (timestamp - this._playbackStartTime) / 1000;
      const newTime = this._timelineStartTime + currentTime;
      
      // Only update if time has changed significantly
      if (Math.abs(this._timeline.currentTime - newTime) >= 0.1) {
